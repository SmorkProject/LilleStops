...
import { Tab, Language, Theme, TrajetInfo } from '@types';
import { TRAJET_PRESETS, colorNames, translations, themes } from '@constants';
            ...
            <View style={style.backgroundBoxAdd}>
              <TouchableWithoutFeedback onPress={() => { console.log("smork") }}>
                <View style={[style.blockBoxAdd, themeFile.color2]}>
                  ...
                  <View>
                    {trajets.map((trajet: TrajetInfo) => (
                      <Pressable
                        key={trajet.id}
                        onPress={async () => {
                          const updatedTrajetsDatas = { ...trajetsDatas };
                          if (trajet.id in updatedTrajetsDatas) {
                            updatedTrajetsDatas[trajet.id].push({
                              type: boxtype,
                              name: boxName,
                              city: boxCity,
                              id: boxId
